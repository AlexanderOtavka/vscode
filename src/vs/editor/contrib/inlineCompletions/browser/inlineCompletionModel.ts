...
export class InlineCompletionModel extends Disposable {
	...
	public update(position: Position, context: InlineCompletionContext, activeInlineCompletion: InlineCompletionWithUpdatedRange | undefined): Promise<boolean> {
		...
		const promise = (async () => {
			...
			transaction(tx => {
				...
				this.inlineCompletions.set(completions, tx);
				if (context.selectedSuggestionInfo) {
					const suggestWidgetsCompletions = new UpToDateInlineCompletions(updatedCompletions, request, this.textModel);
					this.suggestWidgetInlineCompletions.set(suggestWidgetsCompletions, tx);
				}
			});
			this.updateOperation.clear();
...
export class InlineCompletionWithUpdatedRange {
	...
	public isVisible(model: ITextModel, cursorPosition: Position): boolean {
		...
		const cursorPosIndex = Math.max(0, cursorPosition.column - this.updatedRange.startColumn);
